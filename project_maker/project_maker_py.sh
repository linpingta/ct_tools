...
usage(){
	echo "./project_maker.sh -p [project_name] -f [file_name] -m [model_name] -t [file_extension] -d [directory]"
	echo "project_name required, file_name(default as project_name)/model_name(default None)/file_extension(default py)/directory(default current_dir) optional"
	exit 1
}

while getopts ":p:f:hm:t:d:" opt; do
	case $opt in 
	p)
...
echo "Project $PROJECT_PATH Making... in directory: $CUR_DIR"
